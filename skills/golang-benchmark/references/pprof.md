...
Requires `import _ "net/http/pprof"` (see `valeriypushkar/qoder-skills-golang@golang-troubleshooting` skill for secure setup):
...
Short-lived allocations creating GC churn. Objects are allocated and freed rapidly — each one is cheap individually but the aggregate volume triggers frequent GC cycles. Common sources: `fmt.Errorf` in hot paths (allocates every call), interface boxing (`any` arguments), string-to-byte conversions, slice growth without preallocation. → See `valeriypushkar/qoder-skills-golang@golang-performance` skill for allocation reduction patterns.
