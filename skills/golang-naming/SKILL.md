...
Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `golang-linter` skill for configuration and usage.
...
- → See `golang-linter` skill for automated enforcement (revive, predeclared, misspell, errname)
